...
public class MazeSpawner : MonoBehaviour {
	...
	void Start () {
		...
		if (Algorithm == MazeGenerationAlgorithm.PureRecursive)
		{
		...
		else
		{
			...
			for (int row = 0; row < Rows; row++)//generate external left walls
			{
				float x = 0-CellWidth/2;
				float z = row*(CellHeight+(AddGaps?.2f:0));

				Instantiate(Wall,new Vector3(x,0,z)+Wall.transform.position,Quaternion.Euler(0,270,0)).transform.parent = transform;//left
			}
			for (int column = 0; column < Columns; column++)//generate external back walls
			{
				float x = column*(CellWidth+(AddGaps?.2f:0));
				float z = 0-CellHeight/2;

				Instantiate(Wall,new Vector3(x,0,z)+Wall.transform.position,Quaternion.Euler(0,180,0)).transform.parent = transform;// back
			}
		}
